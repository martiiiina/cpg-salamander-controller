...
class RobotParameters(dict):
    ...
    def set_coupling_weights(self, parameters):
        ...
        for a, b in [(16, 20), (24,28), (20,28), (16,24)]: #######################
            self.coupling_weights[a, b] = - w_limb # minus here slightly left then slightly right, without minus it is more straight but slower, with minus it is faster but more left, maybe because the diagonal pairs are in phase and the contralateral pairs are in antiphase, so the minus makes the diagonal pairs more in phase and the contralateral pairs more in antiphase, which produces a stronger trot pattern but also a bit of a left bias
            self.coupling_weights[b, a] = w_limb #putting minus here makes the salamander turn left 
    ...
    def set_phase_bias(self, parameters):
        ...
        for k in [16,20,24,28]:
            ...
            self.phase_bias[k+3, k+2] = np.pi #this is what makes the knee and ankle in antiphase, to produce actual oscillations in the limb, otherwise they would be in phase and the limb would not move

        # Limb: girdle↔knee π/2 offset for circular motion ################
        for leg in [16, 20, 24, 28]:
            self.phase_bias[leg+2, leg] = - np.pi / 2 #if this is the only -np.pi/2 then coordinated motion but backwards
            self.phase_bias[leg, leg+2] = - np.pi / 2
            self.phase_bias[leg+3, leg+1] = - np.pi / 2
            self.phase_bias[leg+1, leg+3] = - np.pi / 2
#FROM NOW ON IT DRIFTS A BIT TO THE LEFT
        # Between limbs: trot
        for a, b in [(16, 20), (24,28), (20,28), (16,24)]: #LOOKS LIKE IT DOESN'T CHANGE MUCH APART FROM SPEED, TRIED ALSO PI/2, unclear
