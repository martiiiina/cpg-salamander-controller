...
def exercise_3a_coordination(timestep):
    ...
    drives         = np.linspace(1.0, 3.0, 5)        # walking drive range [1,3]
    phase_offsets  = np.linspace(-np.pi, np.pi, 9)   # limb-body phase offset
    ...
    sim_parameters = SimulationParameters(
        ...
        phase_lag_body=None,
        w_limb_body=150,
        limb_body_phase_offset=np.deg2rad(phi_deg[best[1]]) + np.pi
    )
    os.makedirs('./logs/ex3.3_walk/', exist_ok=True)
    sim, data = simulation(
        sim_parameters=sim_parameters,
...
def exercise_3b_coordination(timestep):
    ...
    """
    # Parameter sweep ranges
    axial_gains = np.linspace(0, 3, 5)        
    limb_gains  = np.linspace(0, 3, 5)   
    ...
    parameter_set = [
        {
            'sim_parameters': SimulationParameters(
                ...
                phase_lag_body=None,         
                w_limb_body=150,
                limb_body_phase_offset=np.pi/4,        # parameter to sweep
                spawn_position=[0, 0, 0.1],
                spawn_orientation=[0, 0, np.pi/2],
                body_gain=bg,
                limb_gain=lg
    ...
    axes[0].plot(limb_gains[best[1]], axial_gains[best[0]], 'r*', markersize=14,
                 label=f'Best: {fwd_speed[best]:.3f} \nlg={limb_gains[best[1]]:.0f}, bg={axial_gains[best[0]]:.1f}')
    axes[0].legend(fontsize=8)
    ...
    axes[1].plot(limb_gains[best_cot[1]], axial_gains[best_cot[0]], 'w*', markersize=14,
                 label=f'Best: {cot[best_cot]:.3f}\nlg={limb_gains[best_cot[1]]:.0f}, bg={axial_gains[best_cot[0]]:.1f}')
    axes[1].legend(fontsize=8)
    ...
    for i, bg in enumerate(axial_gains):
        axes2[0].plot(limb_gains, fwd_speed[i], color=colors[i], marker='o', label=f'bg={bg:.1f}')
        axes2[1].plot(limb_gains, cot[i],        color=colors[i], marker='o', label=f'bg={bg:.1f}')
